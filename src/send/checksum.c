...
{
    unsigned char ip_size = g_scan.family == AF_INET ? sizeof(t_ipv4_pseudo_header) : sizeof(t_ipv6_pseudo_header);
    ...
    if (g_scan.family == AF_INET)
    {
        t_ipv4_pseudo_header pseudo_header = 
        {
            ...
            .destination_address = IP->destination.addr.in.sin_addr.s_addr,
            .protocol = protocol,
